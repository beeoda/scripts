...
if [ "$(echo $lat | grep -o '[0-9][0-9][NS]')" != "$lat" ]; then
    echo "Error: latitude misspecified"
    usage
fi
if [ "$(echo $lon | grep -o '[0-9][0-9][0-9][EW]')" != "$lon" ]; then
...
for t in $types; do
    echo "  downloading $t"
    wget -q https://storage.googleapis.com/earthenginepartners-hansen/GFC2015/Hansen_GFC2015_${t}_${lat}_${lon}.tif
done
